function [ sp,K,feaAC,feaAS,label] = cal_sp_feature_sift( I_rgb,fileName,sift,opts )
% Input:
...
%         imshow1(I_rgb,sp,h,w);
        ...
        t0 = clock;
        I_rgb = double(I_rgb);
        feaAC = zeros(length(sp),d);
        d_sift = size(sift,3);
        feaAS = zeros(length(sp),d_sift);
        for i = 1:length(sp)
            pixels = sp{i}.pixels;
            n_p = size(pixels,1);
            avg_color = zeros(d,1);color_t = avg_color;
            avg_sift = zeros(d_sift,1);sift_t = avg_sift;
            for j = 1:n_p
                color_t(:) = I_rgb(pixels(j,1),pixels(j,2),:);
                avg_color = avg_color + color_t;
                sift_t(:) = sift(pixels(j,1),pixels(j,2),:);
                avg_sift = avg_sift + sift_t;
            end
            avg_color = avg_color/n_p;
            avg_sift = avg_sift/n_p;
